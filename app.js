/* =============================================
   Center System V15 (QR Auto-Attend Edition)
   Fix: Auto-detect ID from URL for QR Scans
   ============================================= */

document.addEventListener('DOMContentLoaded', () => {
  console.log("System V15 with QR Handler Loaded...");
  ...
  // ====== 11. QR CODE AUTO-HANDLER (الجزء الجديد) ======
  // هنا بنقوله: "بص في الرابط، لو لقيت ID، سجل حضوره فوراً"
  const urlParams = new URLSearchParams(window.location.search);
  const qrId = toInt(urlParams.get("id"));
  
  if (qrId) {
      if(localStorage.getItem(K_AUTH) === "1") {
          // لو الأدمن مسجل دخول، سجل حضور الطالب فوراً
          if(existsId(qrId)) {
              const res = addAttendance(qrId, nowDateStr());
              updateStudentUI(qrId);
              renderReport(nowDateStr());
              
              // رسالة واضحة إن العملية تمت
              const stName = students[qrId].name || "طالب بدون اسم";
              alert(`📷 تم مسح الكود (QR Scan)\n\n👤 الطالب: ${stName}\n🆔 رقم: ${qrId}\n\n✅ الحالة: ${res.msg}`);
              
              // تنظيف الرابط عشان ميسجلوش تاني بالغلط لو عملت ريفريش
              window.history.replaceState(null, null, window.location.pathname);
          } else {
             alert(`❌ خطأ: الطالب رقم ${qrId} غير موجود في السيستم.`);
          }
      } else {
          // لو مش مسجل دخول
          alert("🔒 من فضلك سجل دخول الأول عشان تقدر تسجل حضور بالكود.");
      }
  }
  ...
  // ====== 2. الوظيفة اللي كانت ناقصة (إحياء الزرار) ======
  const makeEmptyStudent = (id) => ({
    id: id, name: "", className: "", phone: "", paid: 0, 
    notes: "", joinedDate: nowDateStr(), attendanceDates: [] 
  });
  ...
  on("addNewBtn", "click", () => {
      const id = toInt($("newId").value);
      // لو الرقم موجود (سواء فاضي أو مليان) امنع الإضافة
      if(!id || existsId(id)) { 
          showMsg("addMsg", "رقم موجود مسبقاً (استخدم البحث)", "err"); 
          return; 
      }
      // لو جديد، ضيفه
      students[String(id)] = makeEmptyStudent(id);
      ...
      saveAll(); 
      window.extOpen(id); 
      showMsg("addMsg", "تمت الإضافة بنجاح", "ok");
  });
